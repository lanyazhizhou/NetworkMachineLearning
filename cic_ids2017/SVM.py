...
def gridSearchCV(Xtrain, Ytrain):
    ...
    clf = svm.SVC()  # 实例化决策树
    GS = GridSearchCV(clf, parameters,n_jobs=3, cv=5)  # 实例化网格搜索，cv指的是交叉验证
...
# 随机抽取比例
raw_data=raw_data.sample(frac=0.02)
...
print("Predict accuracy of SVM: ", np.mean(list_diag) / np.mean(list_raw_sum))

preData = time.time()

preData_end = time.time()
print("加载时间:%fs" %(load_end-load_start))
print("数据处理时间:%fs" %(data_pro-load_end))
print("训练模型时间:%fs" %(trainData_end-data_pro))
print("测试数据时间:%fs" %(preData-trainData_end))
print("总时间:%fs" %(preData-load_start))
